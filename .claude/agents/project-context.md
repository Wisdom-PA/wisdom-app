...
3. **Write the answers into the target files** named by each question's `target`
   ...
   - `ai/shared/00-project.md` — keep the `# app` heading and the
     ...
     pre-init with `app` unresolved, and `tokens.mjs` relies on that
...
- Keep house standards and repo-specifics separate: if an answer restates a rule
  ...
  not belong there.
- Preserve `app`/`none` tokens if still present (pre-init
